...
local SaveManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/wrrexk/pepsi-config-fix/refs/heads/main/savemanger%20rewrite.lua"))()

if not isfolder("kryx.cc//main/kryxlua") then
    makefolder("kryx.cc/main/kryxlua")
end
...
local Tabs = {
    ...
    Extras = Window:AddTab('Extra'),
    Luascripts = Window:AddTab('Luas'), 
}
------------------------------------ SECTIONS ------------------------------------
...
Options.hit_killsoundtype:OnChanged(function()  -- This should be 'hit_killsoundtype' instead of 'hit_hitsoundtype'
...
--[[ExtraSec2:AddToggle('display_namechanged', {Text = 'Name/ID Changer V2', Default = false})
Toggles.display_namechanged:OnChanged(function()
    
end)
ExtraSec2:AddDropdown('display_newname', {Values = {'wrrexk', 'kryx', "Ion CB", "Sjors", "kryx.cc", ".gg/kryx", "pepsi.club", "femboy", "zuhn"}, Default = 1, Multi = false, Text = 'Name (SUPER LAGGY)'})
Options.display_newname:OnChanged(function()

    if Options.display_newname.Value == "wrrexk" then
        namechange = "wrrexk"
        idchange = "1603688493"
    elseif Options.display_newname.Value == "kryx" then
        namechange = "kryx"
        idchange = ""
    elseif Options.display_newname.Value == "ion_lol" then
        namechange = "ion_lol"
        idchange = "619962434"
    elseif Options.display_newname.Value == "Sjors" then
        namechange = "Sjors"
        idchange = "83335331"
    elseif Options.display_newname.Value == "kryx.cc" then
        namechange = "kryx.cc"
        idchange = ""
    elseif Options.display_newname.Value == ".gg/kryx" then
        namechange = ".gg/kryx"
        idchange = ""
    elseif Options.display_newname.Value == "pepsi.club" then
        namechange = "pepsi.club"
        idchange = ""
    elseif Options.display_newname.Value == "femboy" then
        namechange = "femboy"
        idchange = ""
    elseif Options.display_newname.Value == "zuhn" then
        namechange = "zuhnCB"
        idchange = "781247686"
    end
end)]]
...
LuaSec1:AddButton('Load Script', function()
    local selectedScript = Options.kryxlua_dropdown.Value

    if not selectedScript or selectedScript == "" then
        return Library:Notify("Please select a valid Lua script", 2)
    end

    local scriptPath = "kryx.cc/main/kryxlua/" .. selectedScript
    if isfile(scriptPath) then
        local scriptContent = readfile(scriptPath)

        local success, err = pcall(loadstring(scriptContent))
        if success then
            Library:Notify("Loaded script successfully")
        else
            Library:Notify("Error loading script: " .. err, 2)
        end
    else
        Library:Notify("Script file not found", 2)
    end
end)

LuaSec1:AddButton('Refresh List', function()
    kryxluaScripts = {}
    for _, luaFile in pairs(listfiles("kryx.cc/main/kryxlua")) do
        local scriptName = string.gsub(luaFile, "kryx.cc/main/kryxlua\\", "")
        table.insert(kryxluaScripts, scriptName)
    end

    
    Options.kryxlua_dropdown:SetValues(kryxluaScripts)
end)

LuaSec2:AddButton('Unlock All Skins', function() 
    loadstring(game:HttpGet("https://pastebin.com/raw/Qu9inv8N", true))() 
end)

LuaSec2:AddButton('Funswapper', function() 
...
ThemeManager:SetFolder('kryx.cc')
SaveManager:SetFolder('kryx.cc/main')
...
OthersSettings:AddInput('uinamechange', {Default = 'kryx.cc', Numeric = false, Finished = false, Text = 'Window Title', Tooltip = 'Changes window title', Placeholder = '. . .'})
...
local function Misc()
    ...
    if Toggles.display_namechanged.Value then
        local new_name = namechange
        local new_id = idchange 
    ...
    localPlayer.Cash.Value = Toggles.tweaks_cash.Value and 6000 or localPlayer.Cash.Value
end
...
Library:Notify('Finished Loading! Welcome ' ..localPlayer.Name.. ' to kryx.cc by wrrexk on discord!');
